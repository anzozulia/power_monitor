...
ENV_FILE_PROD = .env.docker_production
UID := $(shell id -u)
GID := $(shell id -g)
...
help:
	...
	@echo "  make build-recreate  - Rebuild and recreate (local)"
	...
	@echo "  make build-recreate-prod  - Rebuild and recreate (production)"
	@echo ""
	@echo "Utility:"
...
up-prod:
	UID=$(UID) GID=$(GID) docker compose --env-file $(ENV_FILE_PROD) -f docker-compose.prod.yml up -d

down-prod:
	UID=$(UID) GID=$(GID) docker compose --env-file $(ENV_FILE_PROD) -f docker-compose.prod.yml down

logs-prod:
	UID=$(UID) GID=$(GID) docker compose --env-file $(ENV_FILE_PROD) -f docker-compose.prod.yml logs -f

migrate-prod:
	UID=$(UID) GID=$(GID) docker compose --env-file $(ENV_FILE_PROD) -f docker-compose.prod.yml exec app python src/manage.py migrate

createadmin-prod:
	UID=$(UID) GID=$(GID) docker compose --env-file $(ENV_FILE_PROD) -f docker-compose.prod.yml exec app python src/manage.py createadmin

build-recreate-prod:
	UID=$(UID) GID=$(GID) docker compose --env-file $(ENV_FILE_PROD) -f docker-compose.prod.yml up -d --build --force-recreate
...
clean:
	docker compose -f docker-compose.local.yml down -v --remove-orphans
	UID=$(UID) GID=$(GID) docker compose --env-file $(ENV_FILE_PROD) -f docker-compose.prod.yml down -v --remove-orphans
